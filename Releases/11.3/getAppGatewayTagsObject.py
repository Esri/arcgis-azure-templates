...
def _main(args):
    ...
    if args.dt == "new":
        ...
        if not isBaseDeployment:
            ...
            if not matches:
                tagKey = "[concat('FS@',parameters('serverRole'),'@',parameters('serverContext'),',',parameters('geoeventServerContext'),'@',variables('deploymentId'),'@',parameters('deploymentTimestamp'),'@',parameters('arcgisDeploymentVersion'),'@',parameters('serverFunctions'))]" 
