...
then
    ...
    appGatewayTagsObject=$(python getAppGatewayTagsObject.py --dt existing --tmpf $templateParametersFileName --agpf $appGatewayTagsFileName)
...
az group deployment create --name $deploymentName --mode Incremental --resource-group $resourceGroupName --template-file $templateFileNameTemp --parameters $deploymentParamsTempFile
